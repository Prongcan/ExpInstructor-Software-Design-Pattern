...
Compute mean golden_score & significance_score, correlation, Cohen’s kappa, hit rates, etc.
"""
...
def load_and_process_data(file_path):
    ...
    for item in data:
        ...
        if golden_score_mean is not None and significance_score is not None:
            result[item_id] = {
...
def calculate_metrics(models_data):
    ...
    for name, data in models_data.items():
        ...
        significance = np.array([data[i]['significance_score'] for i in common_ids])
        raw_data[f'{name}_golden'] = golden
        raw_data[f'{name}_significance'] = significance

        corr, p = pearsonr(golden, significance)
        kappa = cohen_kappa_score(np.round(golden).astype(int), np.round(significance).astype(int))
        mae = np.mean(np.abs(significance - golden))
        rmse = np.sqrt(np.mean((significance - golden) ** 2))
        hit1 = np.mean(np.abs(significance - golden) <= 1)
        hit2 = np.mean(np.abs(significance - golden) <= 2)
    ...
    for a, b in itertools.combinations(model_names, 2):
        na = raw_data[f'{a}_significance']
        nb = raw_data[f'{b}_significance']
...
def print_results(results):
    ...
    for name, r in results.items():
        ...
        print(f"  significance Score Mean: {r['significance_mean']:.4f}")
...
def create_visualizations(results, raw_data, output_dir):
    ...
    for name in model_names:
        plt.hist(raw_data[f'{name}_significance'], bins=range(0, 12), alpha=0.5, edgecolor='black', label=name)
    plt.xlabel("significance Score")
    plt.ylabel("Frequency")
    plt.title("significance Score Distribution Across Models")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, '2_significance_distribution.png'), bbox_inches='tight')
    plt.close()
    ...
    for pair in pairs:
        ...
        ax.set_xlabel(f"{a} significance Score")
        ax.set_ylabel(f"{b} significance Score")
        ax.legend()
        ax.set_title(f"{a} vs {b} significance Comparison")
        plt.tight_layout()
...
def main():
    model_paths = {
        "RAG_evidence_retrieval": "Evaluation_significance/results/RAG_evidence_retrieval/success_results.json",
        "Instructor": "Evaluation_significance/results/instructor/success_results.json",
        "GPT": "Evaluation_significance/results/GPT/success_results.json"
    }

    output_dir = "Evaluation_significance/results/compare_three"
    os.makedirs(output_dir, exist_ok=True)
    ...
    if results:
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            ...
            for name, r in results.items():
                ...
                f.write(f"  significance Score Mean: {r['significance_mean']:.4f}\n")
