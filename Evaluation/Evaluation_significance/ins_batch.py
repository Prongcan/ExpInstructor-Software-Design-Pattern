...
from Evaluation.Evaluation_significance.ins_single import generate_significance_evaluation_via_agent
from Evaluation.Evaluation_utils.eval_significance import generate_significance_score
...
def init_worker():
    """Initializes the worker process, creates the agent"""
    global global_agent
...
def process_single_item(item_data):
    ...
    Args:
        item_data: A dictionary containing the following fields:
            - id: Item ID
            - idea: Idea content (can be an empty string)
            - all_comments: Comment content (optional)
            - all_scores: Original scores (optional, format like "3 5")

    Returns:
        A dictionary containing the processing results:
            - id: Item ID
            - status: Status ("success", "error", "skipped")
            - golden_score: Golden standard score (from the all_scores field, format like "3 5")
            ...
            - significance_score: Extracted score (integer from 1-10, or None)
    ...
    try:
        ...
        if not significance_evaluation or significance_evaluation.startswith("ERROR"):
            return {
                ...
                "significance_score": None,
                "significance_score_raw_resp": None,
                "error": "Error during significance evaluation generation"
            }
...
def main():
    # Input file path
    input_file = "Evaluation/Evaluation_significance/significance_data/Stanford_comments_with_ideas_and_scores.json"

    # Output directory
    output_dir = "Evaluation/Evaluation_significance/results/instructor"
