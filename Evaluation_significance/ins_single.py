...
def generate_significance_evaluation_via_agent(agent, idea_text: str) -> str:
    """
    Generates significance evaluation text using an Agent.

    Args:
        agent: An created Agent instance.
        idea_text: The research idea text to be evaluated.

    Returns:
        str: The significance evaluation text.
    """
    # Build the user prompt, using the idea_text directly
    prompt = build_agent_user_prompt(idea_text)

    input_message = HumanMessage(content=prompt)

    # Run the agent once to get the final message content (long report)
    final_text = ""
    step_count = 0
    tool_call_count = 0
    tool_calls_details = []
        ...
        if step.get("messages"):
            ...
            if hasattr(last_message, 'tool_calls') and last_message.tool_calls:
                ...
                for i, tool_call in enumerate(last_message.tool_calls):
                    ...
                    print(f"    Arguments: {json.dumps(tool_args, ensure_ascii=False, indent=4)}")
            ...
            if hasattr(last_message, 'tool_call_id') and last_message.tool_call_id:
                ...
                if last_message.content:
                    # Limit output length to avoid being too long
    ...
    print("\n" + "=" * 80)
    print("Agent execution complete - Statistics")
    print("=" * 80)
    ...
    for i, detail in enumerate(tool_calls_details, 1):
        ...
        print(f"  Arguments: {json.dumps(detail['args'], ensure_ascii=False, indent=4)}")
        if 'result' in detail:
            ...
            print("  Result: [No result obtained]")
    ...
    if not final_text or final_text.strip() == "":
        return "ERROR: Agent did not return a valid result"
...
def main() -> None:
    ...
    result = generate_significance_evaluation_via_agent(agent, raw_idea)
    print(result)
