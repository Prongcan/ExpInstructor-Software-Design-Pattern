...
class RAGEvaluationTemplate(ABC):
    ...
    def __init__(self, evaluation_type: str, embeddings_dir: str = 'Retrive_Generate', retrieval_system=None):
        """
        ...
        Args:
            ...
            retrieval_system: Optional pre-initialized retrieval system
        """
   
        self.evaluation_type = evaluation_type.lower()
        self.embeddings_dir = embeddings_dir
        self._chat_client = get_chat_client()  # Factory Method Pattern
        self._retrieval_system = retrieval_system  # Lazy initialization or injected
    ...
    def _initialize_retrieval_system(self):
        if self._retrieval_system is None:
            ...
            # Use GraphRetrievalSystem instead of EvidenceRetrievalSystem
            # Assuming graph file is in result_v2/all_graphs_cleaned.json relative to project root
            graph_file = os.path.join(PROJECT_ROOT, 'result_v2', 'all_graphs_cleaned.json')
            self._retrieval_system = GraphRetrievalSystem(graph_file)
            
            # Display statistics
            # GraphRetrievalSystem doesn't have get_statistics method, using GraphPresenter.show_stats if needed
            # or just printing basic info
            print(f"[📊 Stats] Graph Retrieval System initialized with file: {graph_file}")
    ...
    def _search_evidence_with_retrieval_system(self, keywords: t.List[str], retrieval_system, top_k=5) -> t.List[str]:
        ...
        for i, keyword in enumerate(keywords, 1):
            ...
            try:
                ...
                # GraphRetrievalSystem uses smart_search or semantic_vector_search
                results = retrieval_system.smart_search(keyword, top_k=top_k)
                
                keyword_results = []
                # Adapt GraphRetrievalSystem results to expected format
                if 'evidence' in results:
                    for edge in results['evidence']:
                        # edge is a tuple (source, target, attrs) or dict depending on implementation
                        # Based on smart_search implementation, it returns a dict with 'evidence' list
                        # where each item is (source, target, attrs)
                        if isinstance(edge, tuple) and len(edge) >= 3:
                            source, target, attrs = edge
                            evidence_text = attrs.get('evidence', '')
                            paper_id = attrs.get('paper_id', 'Unknown')
                            review_id = attrs.get('review_id', 'Unknown')
                            similarity = 0.0 # Similarity not directly available in this format
                            
                            entry = f"📘 Paper ID: {paper_id} | Review ID: {review_id} | Similarity: {similarity:.3f}\n"
                            entry += f"{evidence_text}\n"
        ...
        for result in all_results:
            ...
            try:
                ...
                if result not in seen_evidence:
                    seen_evidence.add(result)
                    unique_results.append(result)
...
def create_feasibility_evaluator(embeddings_dir: str = 'Retrive_Generate') -> RAGEvaluationTemplate:
...
def create_novelty_evaluator(embeddings_dir: str = 'Retrive_Generate') -> RAGEvaluationTemplate:
    """Create RAG novelty evaluator. Refactored with Template Method Pattern."""
    return RAGEvaluationTemplate("novelty", embeddings_dir)

def create_significance_evaluator(embeddings_dir: str = 'Retrive_Generate') -> RAGEvaluationTemplate:
...
def create_feasibility_score_evaluator(embeddings_dir: str = 'Retrive_Generate') -> RAGEvaluationTemplate:
...
def generate_rag_evaluation(idea_text: str, evaluation_type: str, embeddings_dir: str = 'Retrive_Generate') -> t.Union[str, t.List[str]]:
    """
